...
$scope.updateRows = []; 
 $scope.columnDefs = [
     ...
     {headerName:"ProcessID",field:"ProcessID",width:100},
    ...
    $scope.gridOptions = {
        ...
        //selectionChanged: rowSelectedFunc, 
    ...
    function cellUpdatedHandler(params) {
        if(params.oldValue !== params.newValue && $scope.updateRows.indexOf(params.rowIndex) < 0) {
            $scope.updateRows.push(params.rowIndex);
        }
        console.log($scope.updateRows);
      };
    ...
    $scope.secOptions = {
        ...
        groupUseEntireRow: true
        // ready: function() {
        //     $scope.secOptions.api.sizeColumnsToFit()
        // }
    }; 
    
    // $scope.filter = $scope.secOptions.api.getFilterApi('NpiID'); 
     $scope.filterModel =[]; 
    // $scope.filter.setModel($scope.filterModel); 
